...
// 3D Robot Component
const Robot3D = ({ position, rotation, scale, animated = true }: {
  ...
  return (
    ...
    >
      ...
      {/* Arms */}
      <Cylinder args={[0.1, 0.1, 0.8]} position={[-0.8, 0.8, 0]} rotation={[0, 0, Math.PI / 6]}>
      ...
      {/* Wheels */}
      <Cylinder args={[0.3, 0.3, 0.2]} position={[-0.6, -0.3, 0.6]} rotation={[Math.PI / 2, 0, 0]}>
...
const IoTDevice3D = ({ position, rotation, scale, type = "sensor" }: {
  position: [number, number, number];
  rotation: [number, number, number];
  scale: number;
  ...
  return (
    ...
    >
      ...
      <Box args={[1, 0.6, 0.8]} position={[0, 0, 0]}>
        <meshStandardMaterial color={getDeviceColor()} metalness={0.7} roughness={0.3} />
      </Box>
      ...
      {/* Gateway Dish */}
      {type === "gateway" && (
        <Torus args={[0.3, 0.02]} position={[0, 0.4, 0]} rotation={[Math.PI / 2, 0, 0]}>
          <meshStandardMaterial color="#1F2937" metalness={0.8} roughness={0.2} />
        </Torus>
      )}
    </group>
...
const BlockchainNetwork3D = ({ position, scale }: {
  position: [number, number, number];
  scale: number;
}) => {
...
const Project3DPreview = ({ project, className = "" }: {
  project: ProjectType;
  className?: string;
}) => {
      ...
      case "robot":
        return (
          <>
            ...
            <ContactShadows position={[0, -2, 0]} scale={10} blur={2} far={20} />
          </>
        );
      case "iot":
        return (
          <>
            <IoTDevice3D position={[-1.5, 0, 0]} rotation={[0, 0, 0]} scale={0.6} type="sensor" />
            <IoTDevice3D position={[0, 0, 0]} rotation={[0, Math.PI / 4, 0]} scale={0.6} type="gateway" />
            <IoTDevice3D position={[1.5, 0, 0]} rotation={[0, Math.PI / 2, 0]} scale={0.6} type="actuator" />
            <Environment preset="city" />
            <ContactShadows position={[0, -1, 0]} scale={8} blur={1.5} far={15} />
          </>
        );
      case "research":
        return (
        ...
        return (
          <>
            <Box args={[1, 1, 1]} position={[0, 0, 0]}>
              <meshStandardMaterial color={project.accentColor} metalness={0.7} roughness={0.3} />
            </Box>
  ...
  return (
    <div className={`relative w-full h-64 lg:h-80 rounded-xl overflow-hidden ${className}`}>
      <Canvas
        ref={canvasRef}
      ...
      >
        ...
        <pointLight position={[-5, 5, 5]} intensity={0.5} color={project.accentColor} />
      ...
      <div className="relative z-10 p-6 lg:p-8">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {project.keyFeatures.slice(0, isExpanded ? undefined : 3).map((feature, idx) => (
              <motion.div
                key={idx}
          ...
          {project.keyFeatures.length > 3 && (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
            >
              {isExpanded ? "Show Less" : `+${project.keyFeatures.length - 3} More`}
            </motion.button>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            {project.metrics.map((metric, idx) => (
              ...
              >
                ...
                <div className="font-orbitron text-lg font-bold text-white">{metric.value}</div>
                <div className="font-inter text-xs text-gray-400">{metric.label}</div>
              </motion.div>
            ))}
          </div>
        )}
